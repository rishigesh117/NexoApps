...
import { Search, Bell, Shield, LogOut, LayoutDashboard, ExternalLink } from 'lucide-react';
import { NotificationDropdown } from './pipeline/NotificationDropdown';
  ...
  return (
    <header className="sticky top-0 z-30 glass-panel border-b border-white/10 px-4 sm:px-8 py-3.5 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-brand-cyan to-brand-violet flex items-center justify-center text-slate-950 font-black text-xs shadow-glow-cyan">
          N
        </div>
        <div>
          <span className="text-sm font-extrabold text-white tracking-wide">Google Play Console</span>
          <span className="text-[10px] text-brand-cyan font-semibold block">NexoApps Admin v3.5</span>
        </div>
      </div>
      ...
      <div className="hidden md:flex items-center relative w-80">
        ...
        <input
          type="text"
          placeholder="Search apps, users, developers..."
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 pl-2 border-l border-white/10">
          ...
          <div className="hidden lg:block text-left text-xs">
            <span className="font-bold text-white block leading-tight">{user?.username || 'Admin'}</span>
